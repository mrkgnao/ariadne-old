...
import           Data.Void
import qualified Edible.Internal.PackMap             as OI
import qualified Edible.Internal.TableMaker          as OI
...
import qualified Edible.Internal.Column              as OI (Column(..), unColumn)
...
import qualified Edible.Internal.Profunctors         as PP
import           Edible.Internal.Record              (Record (RCons, RNil))
import qualified Edible.Internal.Record              as Record
...
-- @
data OTable writerColumns viewColumns
  = OTable String (TableProperties writerColumns viewColumns)
    -- ^ Uses the default schema name (@\"public\"@).
  | OTableWithSchema String String (TableProperties writerColumns viewColumns)
    -- ^ Schema name (@\"public\"@ by default in PostgreSQL), table name,
    --   table properties.

tableIdentifier :: OTable writerColumns viewColumns -> PQ.TableIdentifier
tableIdentifier (OTable t _)             = PQ.TableIdentifier Nothing t
tableIdentifier (OTableWithSchema s t _) = PQ.TableIdentifier (Just s) t

tableProperties :: OTable writerColumns viewColumns -> TableProperties writerColumns viewColumns
tableProperties (OTable _ p)             = p
tableProperties (OTableWithSchema _ _ p) = p
...
queryTable :: TM.ColumnMaker viewColumns columns
            -> OTable writerColumns viewColumns
            -> Tag.Tag
...
required :: String -> Writer (OI.Column a) (OI.Column a)
required columnName =
  Writer (PM.PackMap (\f columns -> f (fmap OI.unColumn columns, columnName)))

optional :: String -> Writer (Maybe (OI.Column a)) (OI.Column a)
optional columnName =
  Writer (PM.PackMap (\f columns -> f (fmap maybeUnColumn columns, columnName)))
  where maybeUnColumn Nothing       = HPQ.DefaultInsertExpr
        maybeUnColumn (Just column) = OI.unColumn column
...
instance Functor (OTable a) where
  fmap f (OTable t tp)             = OTable t (fmap f tp)
  fmap f (OTableWithSchema s t tp) = OTableWithSchema s t (fmap f tp)
...
--
-- * @pgType@: Type of the column value used in Edible queries as index to
--   'Kol' or 'Koln'. This must be an instance of 'PgTyped'.
--
-- * @hsType@: Type of the column value used in Haskell outside Edible
...
-- the database, as taken by "Edible.Run.runInsert".
--
...
---
-- | Output type of @'Edible.query' (... :: 'Table' t)@.
--
...
-- type can be used as input for "Edible.Run.runInsertRaw" and friends.
--
...
-- "Edible.Internal.Record") are sufficient to you, this is
-- just a convenience.
...
-- it as a new row using 'Edible.Run.runInsert'.
...
-- to a @('PgW' t)@ that can be used in a 'Edible.runUpdate'-like
-- operation.
...
colProps_ro :: PgTyped a => String -> TableProperties Void (Kol a)
colProps_ro n = P.rmap Kol $ TableProperties
  (Writer (OI.PackMap (\f ws -> f (fmap absurd ws, n)))) -- <- dead code
  (View (OI.runViewColumnMaker OI.tableColumn n))

-- | Column properties: Read only (nullable).
colProps_ron :: PgTyped a => String -> TableProperties Void (Koln a)
colProps_ron n = P.rmap Koln $ TableProperties
  (Writer (OI.PackMap (\f ws -> f (fmap absurd ws, n)))) -- <- dead code
  (View (OI.runViewColumnMaker OI.tableColumn n))

-- | Column properties: Write (no default), Read (not nullable).
colProps_wr :: PgTyped a => String -> TableProperties (Kol a) (Kol a)
colProps_wr = P.dimap unKol Kol . orequired

-- | Column properties: Write (no default), Read (nullable).
colProps_wrn :: PgTyped a => String -> TableProperties (Koln a) (Koln a)
colProps_wrn = P.dimap unKoln Koln . orequired

-- | Column properties: Write (optional default), Read (not nullable).
colProps_wdr :: PgTyped a => String -> TableProperties (WDef (Kol a)) (Kol a)
colProps_wdr = P.dimap (wdef Nothing Just . fmap unKol) Kol . ooptional

-- | Column properties: Write (optional default), Read (nullable).
colProps_wdrn :: PgTyped a => String -> TableProperties (WDef (Koln a)) (Koln a)
colProps_wdrn = P.dimap (wdef Nothing Just . fmap unKoln) Koln . ooptional

-- | 'required' is for columns which are not 'optional'.  You must
-- provide them on writes.
orequired :: String -> TableProperties (OI.Column a) (OI.Column a)
orequired columnName = TableProperties
  (required columnName)
  (View (OI.Column (HPQ.BaseTableAttrExpr columnName)))

-- | 'optional' is for columns that you can omit on writes, such as
--  columns which have defaults or which are SERIAL.
ooptional :: String -> TableProperties (Maybe (OI.Column a)) (OI.Column a)
ooptional columnName = TableProperties
  (optional columnName)
  (View (OI.Column (HPQ.BaseTableAttrExpr columnName)))

--------------------------------------------------------------------------------

-- | 'TableProperties' for a single column in @'Columns' t@.
type family Column_Props (col :: Column Symbol WCap RCap Type Type) :: Type where
  Column_Props ('Column n 'RO r p h) =
    TableProperties Void (Column_PgR ('Column n 'RO r p h))
  Column_Props col = TableProperties (Column_PgW col) (Column_PgR col)
...
-- | A wrapper around @opaleye@'s 'OTable' adding a placeholder @t@, which
-- shall mention the 'Database' associated with the query.
newtype RawTable (d :: k) w r = RawTable { unRawTable :: OTable w r }
  deriving (Functor)

-- | Obtain the read-write 'RawTable' for a 'Table', required by
-- 'Edible.Run.runInsertRaw' and friends.
rawTableRW :: TableRW t => Table t -> RawTable (Database t) (PgW t) (PgR t)
rawTableRW (_ :: Table t) = RawTable $ OTableWithSchema
...
-- interact with the underlying Edible library.
rawTableRO :: TableR t => Table t -> RawTable (Database t) Void (PgR t)
rawTableRO (_ :: Table t) = RawTable $ OTableWithSchema
  (symbolVal (Proxy :: Proxy (SchemaName t)))
  (symbolVal (Proxy :: Proxy (TableName t)))
